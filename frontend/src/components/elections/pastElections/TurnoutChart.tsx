...
const TurnoutChart: React.FC<TurnoutChartProps> = ({ data }) => {
  ...
  const minPercentage = Math.max(40, Math.min(...data.map(item => item.percentage)) - 10);
  const maxPercentage = Math.min(100, Math.max(...data.map(item => item.percentage)) + 10);
  ...
  return (
    <View className="bg-white rounded-lg p-4 mb-4">
      <Text className="font-bold text-gray-800 mb-4">Turnout Trends</Text>
